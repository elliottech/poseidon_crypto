		},
		{
			x: gFp5.Element{11575997426281090678, 1534495174840625570, 7539338128385981583, 10393042019577161985, 10667466219175771157},
			u: gFp5.Element{16681365912970185037, 11287896019745355117, 11069899752345274504, 15487604769605237513, 13467978440572613228},
		},
		{
			x: gFp5.Element{11192179397773394280, 3555953455665397909, 5346523552109387121, 4514445299325204396, 3932728981135688453},
			u: gFp5.Element{5421638117266109845, 204299445119713184, 6067390115784997081, 16191134954342419157, 4139938600224417293},
		},
		{
			x: gFp5.Element{13189785832536261642, 8777097377506996162, 17497140949916325738, 15140279769427597032, 15517274717131999881},
			u: gFp5.Element{1040464435413162742, 9262701069034606854, 2990438819650713743, 18129195737333990255, 12490074042478236606},
		},
		{
			x: gFp5.Element{17716508479149156535, 14351380558651795729, 3644546258883003807, 5171318241596472386, 294806796132518330},
			u: gFp5.Element{7535225611936271281, 14682077054502188499, 784215514926156349, 5280586574139275596, 14407528916988559545},
		},
		{
			x: gFp5.Element{8681294642569802563, 7751765660802747503, 16382129702876313971, 7447155060842833278, 6859908403876474879},
			u: gFp5.Element{9674486254207846385, 5248970165164951259, 3611784478790504991, 18437168019170350173, 3537959913875671086},
		},
	}
)
...
func GenerateG() {
	...
	res := BatchToAffine(tmp)
	for i := 0; i < 16; i++ {
		elx := fmt.Sprintf("gFp5.Element{%d, %d, %d, %d, %d}", res[i].x[0], res[i].x[1], res[i].x[2], res[i].x[3], res[i].x[4])
		elu := fmt.Sprintf("gFp5.Element{%d, %d, %d, %d, %d}", res[i].u[0], res[i].u[1], res[i].u[2], res[i].u[3], res[i].u[4])
		fmt.Printf("AffinePoint{\n    x : %s,\n    u : %s,\n},\n", elx, elu)
	}
}
...
func MulAddG(b ECgFp5Point, scalarA, scalarB ECgFp5Scalar) ECgFp5Point {
	...
	winB := b.MakeWindowAffine()

	digitsA := make([]int32, (319+WINDOW)/WINDOW)
	scalarA.RecodeSigned(digitsA, int32(WINDOW))
	digitsB := make([]int32, (319+WINDOW)/WINDOW)
	scalarB.RecodeSigned(digitsB, int32(WINDOW))

	n := len(digitsA) // == len(digitsB)

	// Top window digit of each scalar, combined into the running point.
	p := LookupVarTime(winA, digitsA[n-1]).ToPoint()
	p = p.AddAffine(LookupVarTime(winB, digitsB[n-1]))

	for i := n - 2; i >= 0; i-- {
		p.SetMDouble(uint32(WINDOW)) // ×2^WINDOW, shared by both scalars
		p = p.AddAffine(LookupVarTime(winA, digitsA[i]))
		p = p.AddAffine(LookupVarTime(winB, digitsB[i]))
	}
	return p
}
