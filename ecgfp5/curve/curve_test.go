...
func TestToAffineAndLookup(t *testing.T) {
	...
	for n := 1; n <= len(tab1); n++ {
		...
		for i := 0; i < n; i++ {
			if !gFp5.Fp5Equals(gFp5.Fp5Mul(tab1[i].z, tab2[i].x), tab1[i].x) {
				t.Fail()
			}
			if !gFp5.Fp5Equals(gFp5.Fp5Mul(tab1[i].t, tab2[i].u), tab1[i].u) {
				t.Fail()
			}
		}
	}
	...
	if !gFp5.Fp5Equals(p1Affine.x, gFp5.FP5_ZERO) {
		t.Fatalf("Lookup failed for 72: expected %v, got %v", gFp5.FP5_ZERO, p1Affine.x)
	}
	if !gFp5.Fp5Equals(p1Affine.u, gFp5.FP5_ZERO) {
		t.Fatalf("Lookup failed for 72: expected %v, got %v", gFp5.FP5_ZERO, p1Affine.u)
	}

	for i := 1; i <= 8; i++ {
		p2Affine := Lookup(win, int32(i))
		if !gFp5.Fp5Equals(gFp5.Fp5Mul(tab1[i-1].z, p2Affine.x), tab1[i-1].x) {
			t.Fatalf("Lookup failed for %d: expected %v, got %v", i, tab1[i-1].x, gFp5.Fp5Mul(tab1[i-1].z, p2Affine.x))
		}
		if !gFp5.Fp5Equals(gFp5.Fp5Mul(tab1[i-1].t, p2Affine.u), tab1[i-1].u) {
			t.Fatalf("Lookup failed for %d: expected %v, got %v", i, tab1[i-1].u, gFp5.Fp5Mul(tab1[i-1].t, p2Affine.u))
		}

		p3Affine := Lookup(win, int32(-i))
		if !gFp5.Fp5Equals(gFp5.Fp5Mul(tab1[i-1].z, p3Affine.x), tab1[i-1].x) {
			t.Fatalf("Lookup failed for -%d: expected %v, got %v", i, tab1[i-1].x, gFp5.Fp5Mul(tab1[i-1].z, p3Affine.x))
		}
		if !gFp5.Fp5Equals(gFp5.Fp5Mul(tab1[i-1].t, p3Affine.u), gFp5.Fp5Neg(tab1[i-1].u)) {
			t.Fatalf("Lookup failed for -%d: expected %v, got %v", i, gFp5.Fp5Neg(tab1[i-1].u), gFp5.Fp5Mul(tab1[i-1].t, p3Affine.u))
		}
	}
}
...
func TestBasicOps(t *testing.T) {
	...
	if !gFp5.Fp5Equals(p0.Encode(), vectors[0]) || !gFp5.Fp5Equals(p1.Encode(), vectors[1]) || !gFp5.Fp5Equals(p2.Encode(), vectors[2]) || !gFp5.Fp5Equals(p3.Encode(), vectors[3]) || !gFp5.Fp5Equals(p4.Encode(), vectors[4]) || !gFp5.Fp5Equals(p5.Encode(), vectors[5]) || !gFp5.Fp5Equals(p6.Encode(), vectors[6]) || !gFp5.Fp5Equals(p7.Encode(), vectors[7]) {
		t.Fatalf("Encoding checks failed")
	}

	if !gFp5.Fp5Equals(p1.Add(p2).Encode(), vectors[3]) || !gFp5.Fp5Equals(p1.Add(p1).Encode(), vectors[4]) || !gFp5.Fp5Equals(p2.Double().Encode(), vectors[5]) || !gFp5.Fp5Equals(p1.Double().Add(p2).Encode(), vectors[6]) || !gFp5.Fp5Equals(p1.Add(p2).Add(p2).Encode(), vectors[7]) {
		t.Fatalf("Addition and doubling checks failed")
	}

	if !gFp5.Fp5Equals(p0.Double().Encode(), gFp5.FP5_ZERO) || !gFp5.Fp5Equals(p0.Add(p0).Encode(), gFp5.FP5_ZERO) || !gFp5.Fp5Equals(p0.Add(p1).Encode(), vectors[1]) || !gFp5.Fp5Equals(p1.Add(p0).Encode(), vectors[1]) {
	...
	p2Affine := AffinePoint{
		x: gFp5.Fp5Mul(p2.x, gFp5.Fp5InverseOrZero(p2.z)),
		u: gFp5.Fp5Mul(p2.u, gFp5.Fp5InverseOrZero(p2.t)),
	}
